...
class Streamer:
    ...
    def _send_loop_forever(self) -> None:
        while True:
    ...
    def run(self) -> None:
        import threading
        t = threading.Thread(target=self._produce_forever, daemon=True)
        t.start()
        self._send_loop_forever()
